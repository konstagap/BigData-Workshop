...
upperRange := [90000, 80000, 75000, 23500, 31450];
...
//Print all avgHouseIncome that are not in lowerRange
lowRangeIncome := empDS(avgHouseIncome NOT IN lowerRange);
OUTPUT(lowRangeIncome, NAMED('lowRangeIncome'));

//Print all avgHouseIncome that are  in upperRange
// Call it upperRangeIncome
OUTPUT(empDS(avgHouseIncome  IN upperRange), 
...
//Calculate the average income of all employed people.
//Round result by 2 decimal points
//Name it: roundIt
isEmp   := empDS(isEmployed);
getAvg  := AVE(isEmp, avgHouseIncome);
roundit := ROUND(getAvg, 2);
OUTPUT(roundIt, NAMED('roundIt'));

//Get max & min value of empGroupNum
//Print the result like following:
//Max Group Number:   , Min Group Num: 
//Mame it: Max_Min_GroupNum
getMax := MAX(empDS, empGroupNum);
getMin := MIN(empDS, empGroupNum);
OUTPUT('Max Group Number: ' + getMax +
       ' , Min Group Num: ' + getMin,
       NAMED('Max_Min_GroupNum'));
